...
(function (global, factory) {
  ...
  var divolteDefined = "object" === typeof window['divolte'];
  info("Checking if Divolte object has been defined, result=" + divolteDefined);

  var divolteLoaded = (divolteDefined && window['divolte']['loaded'] === true);
  info("Checking if Divolte has finished loading, result=" + divolteLoaded);

  if (!divolteDefined || !divolteLoaded) {
    // load event queue if it exists (divolte has not finished loading)
    var eventQueue = [];
    if(divolteDefined && !divolteLoaded) {
      eventQueue = window['divolte']['queue'];
      log("Event queue contains " + eventQueue.length + " event(s)");
    }
    ...
    for(var i = 0; i < eventQueue.length; i++) {
      ...
      var type = entry[0];
      var customParameters = entry[1];

      if ('undefined' === typeof customParameters) {
        info("Dequeuing event from event queue: " + type);
      } else {
        info("Dequeuing event from event queue: " + type, customParameters);
      }

      signal(type, customParameters);
    }
  } else {
